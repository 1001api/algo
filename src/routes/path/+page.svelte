<script lang="ts">
	...
	import { PriorityQueue, type Edge } from "../../data/pq";
	import { Queue } from "../../data/q";
	...
	function dijkstra() {
		...
		while (pq.heap.length) {
			...
			for (let i = 0; i < neighbors.length; i++) {
				...
				const calculated =
					distance[`${temp.y},${temp.x}`] + current.weight;
	...
	function dfs(vertex: Edge) {
		...
		(function helper(v: Edge, path: Edge[]) {
			...
			for (let i = 0; i < neighbors.length; i++) {
				...
				if (!visited.has(`${val.y},${val.x}`) && !result.length)
					helper(val, [...path, v]);
			}
		})(vertex, []);
		...
		<div class="px-2">
			<div class="form-control w-full max-w-xs">
				<select bind:value={gridAlgo} class="select-bordered select">
					...
					<option value="dijkstra" selected
